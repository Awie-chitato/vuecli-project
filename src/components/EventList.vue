<template>
  <div class="event">
    <div> 
    <button @click="showInputForm = true; resetForm();" class="add-button" style="float: left; ">Tambah</button>
    </div>
    <!-- <h2 class="dashboard-title">Event List</h2> -->
    <div class="transaction-container">
      <div v-if="showInputForm" class="input-form">
        <h3>{{ editMode ? 'Edit Event' : 'Form Input Event' }}</h3>
        <form @submit.prevent="submitForm">
          ...
          <div class="form-group">
            <label for="tempatEvent">Nama Tempat:</label>
          ...
          <tbody>
            <tr v-for="(event, index) in eventlist" :key="index">
              ...
              <td class="action-column">
                <button @click="showTransactionTypeModal = true; event_id_transaction = event.id" class="add-transaction" style="background-color: #28a745; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;margin-right: 15px">Transaksi</button>
                <button @click="editItem(event)" class="action-button edit" style="background-color: #ffc107; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-right: 15px;">Edit</button>
                <button @click="deleteItem(event)" class="action-button delete" style="background-color: #dc3545; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-right: 15px;">Hapus</button>
      ...
      <div v-if="showInputTransactionForm" class="input-form">
        ...
        <form @submit.prevent="submitTransactionForm">
          ...
          <button type="button" @click="cancelTransactionForm" class="cancel-button">Batal</button>
        </form>
      </div>
    </div>
  </div>
